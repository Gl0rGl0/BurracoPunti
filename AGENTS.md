...
- **Backend Desktop:** `main.py` usa `pywebview` per racchiudere l'interfaccia web in una finestra nativa Windows ed esporre l'API di salvataggio automatico su `torneo_data.json` e il file dialog nativo per l'export Excel.
...
- `torneo_data.json` — Struttura JSON multi-giornata indicizzata per data (`giornata_GGMMAA`).
...
3. **Persistenza Dati:** Le serate sono archiviate in `torneo_data.json` con prefisso data `giornata_GGMMAA` (es. `giornata_040926` per il 4 settembre 2026).
