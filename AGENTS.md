...
- **Backend Desktop:** `main.py` usa `pywebview` per racchiudere l'interfaccia web in una finestra nativa Windows ed esporre l'API di salvataggio automatico su `statistiche_tornei.json` (salvato in `C:\Users\[UTENTE]\Documents\BurracoPezzo`) e il file dialog nativo per l'export Excel.
...
- `statistiche_tornei.json` — Struttura JSON multi-giornata indicizzata per data (`serata_GGMMAA`), persistita in `C:\Users\[UTENTE]\Documents\BurracoPezzo\statistiche_tornei.json` con fallback/migrazione locale.
...
3. **Persistenza Dati:** Le serate sono archiviate in `statistiche_tornei.json` (sotto `Documents\BurracoPezzo`) con prefisso data `serata_GGMMAA` (es. `serata_040926` per il 4 settembre 2026).
